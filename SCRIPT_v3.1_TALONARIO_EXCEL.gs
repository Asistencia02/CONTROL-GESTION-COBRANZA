// ==================== GOOGLE APPS SCRIPT v3.1 - TALONARIO DEL EXCEL ====================
// ✅ FIX: Lee el talonario del Excel y lo usa en pagos
...
function sincronizarDesdeGoogleSheets() {
  try {
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🚀 SINCRONIZACIÓN DESDE GOOGLE SHEETS v3.1 (TALONARIO DEL EXCEL)");
...
function sincronizarInteligente() {
  try {
    ...
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🚀 v3.1 - SINCRONIZACIÓN INTELIGENTE (TALONARIO DEL EXCEL)");
    Logger.log("=".repeat(80));
...
function procesarHojaCobranzaConValidacionDNI(hoja, nombreHoja, carreraId, instId, año, datosInst, dniYaVistosEnInstitucion) {
  ...
  for (let r = 1; r < datos.length; r++) {
    ...
    // ✅ LEER TALONARIO GENERAL (la primera columna TALONARIO que encuentre)
    ...
    if (montoPagadoInsc > 0) {
      conceptosPagados.push({
        ...
        metodo: String(fila[idxInscMetodo] || "EFECTIVO").trim().toUpperCase(),
        talonario: talonarioGeneral || String(fila[idxInscTalonario] || "").trim()
      });
    }
    ...
    for (let mes in idxCuota) {
      ...
      if (montoPagado > 0) {
        conceptosPagados.push({
          ...
          metodo: String(fila[idxCuotaMetodo[mes]] || "EFECTIVO").trim().toUpperCase(),
          talonario: talonarioGeneral || String(fila[idxCuotaTalonario[mes]] || "").trim()
        });
      }
    }
    ...
    for (let mes in idxSeguro) {
      ...
      if (montoPagado > 0) {
        conceptosPagados.push({
          ...
          mes: mes,
          montoPagado: montoPagado,
          metodo: String(fila[idxSeguroMetodo[mes]] || "EFECTIVO").trim().toUpperCase(),
          talonario: talonarioGeneral || String(fila[idxSeguroTalonario[mes]] || "").trim()
        });
      }
    }
...
// ==================== CREAR PAGO MÚLTIPLE (CON TALONARIO DEL EXCEL) ====================
...
function crearPagoMultipleAgrupado(config, estId, estudiante, conceptosPagados, conceptos, config_carreras, carreraId) {
  try {
    ...
    // ✅ USAR TALONARIO DEL EXCEL
    const talonario = estudiante.talonario || `EST_${estId}_${instId}_2025`;
