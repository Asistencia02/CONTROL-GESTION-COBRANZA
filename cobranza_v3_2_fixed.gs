// ==================== GOOGLE APPS SCRIPT v3.2 - BUG FIX ====================
// ✅ v3.2: FIX para detectar celdas vacías en CUOTAS/SEGUROS
// ✅ Validación estricta: solo grabar conceptos que REALMENTE existen en Excel
// ✅ Logging detallado para auditar qué se graba
...
function esCeldaVacia(valor) {
  ...
  return str === "" || str === " " || str === "0" && parseInt(valor) === 0;
}
...
function onOpen() {
  try {
    ...
    ui.createMenu('🔄 SINCRONIZACIÓN COBRANZA v3.2')
      .addItem('▶️ SINCRONIZAR (VALIDADO - DRY RUN)', 'sincronizarDryRun')
      .addItem('▶️ SINCRONIZAR (VALIDADO - REAL)', 'sincronizarInteligente')
      .addSeparator()
...
// ==================== DRY RUN (SOLO ANÁLISIS, NO GRABA) ====================
...
function sincronizarDryRun() {
  try {
    ...
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🔍 v3.2 - DRY RUN (ANÁLISIS SIN GRABAR)");
    ...
    const resultado = sincronizarCobranzaInteligente(true); // ← dryRun = true
    ...
    resumen += `• Conceptos sospechosos detectados: ${STATS_GLOBAL.conceptosCuestionables}\n`;
    resumen += `• Tiempo: ${(elapsed/1000).toFixed(2)}s\n\n`;
    resumen += `✅ NADA FUE GRABADO. Revisa 'AUDIT_CONCEPTOS_GRABADOS' para validar.\n`;
    resumen += `Si los números son correctos, ejecuta 'SINCRONIZAR (VALIDADO - REAL)'`;
...
function sincronizarInteligente() {
  try {
    ...
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🚀 v3.2 - SINCRONIZACIÓN VALIDADA (REAL)");
    Logger.log("=".repeat(80));
    ...
    const resultado = sincronizarCobranzaInteligente(false); // ← dryRun = false
...
function procesarCobranzaPorInstitucionInteligente(config, datosInst, dryRun = false) {
  ...
  for (let i = 0; i < pagosConId.length; i += PAGO_PARALLEL_BATCH) {
    ...
    for (let pago of lote) {
      // ✅ USAR ID SIMULADO EN DRY RUN
      const estId = dryRun ? pago.estudiante.item || Math.floor(Math.random() * 10000) : pago.estudiante.item;
...
function procesarHojaCobranzaConValidacionDNI(hoja, nombreHoja, carreraId, instId, año, datosInst, dniYaVistosEnInstitucion) {
  ...
  const mesesExpandidos = ["MARZO", "ABRIL", "MAYO", "JUNIO", "JULIO", "AGOSTO", "SEPTIEMBRE", "OCTUBRE", "NOVIEMBRE", "DICIEMBRE"];
  ...
  let procesados = 0, conConceptos = 0, duplicadosRechazados = 0, saltadosPorValidacion = 0;
  ...
  for (let r = 1; r < datos.length; r++) {
    ...
    // ✅ VALIDACIÓN ESTRICTA: CUOTAS
    for (let mes of Object.keys(idxCuota)) {
      const idx = idxCuota[mes];
      const celdaValor = fila[idx];
      
      // ✅ VALIDACIÓN CRÍTICA: Si la celda está vacía, SALTAR
      ...
      if (montoPagado > 0) {
        ...
        // 📊 AUDIT: Registrar cuota detectada
    ...
    for (let mes of Object.keys(idxSeguro)) {
      ...
      if (montoPagado > 0) {
        ...
        // 📊 AUDIT: Registrar seguro detectado
        AUDIT_CONCEPTOS.push({
          hoja: nombreHoja,
          dni: dniRaw,
          apellido: apellido,
...
function cargarConfiguracionCarreras(config) {
  ...
  try {
    ...
    logConTiempo(`   ❌ Error cargando config: ${e}`);
  }
...
function crearPagoMultipleAgrupado(config, estId, estudiante, conceptosPagados, conceptos, config_carreras, carreraId, dryRun = false) {
  try {
    ...
    if (dryRun) {
      // ✅ DRY RUN: Solo registrar en AUDIT, no grabar
      AUDIT_CONCEPTOS.push({
        hoja: "(PAGO)",
        dni: estudiante.dni,
        ...
        concepto_mes: null,
        monto: montoTotal,
        celda_valor: `${detalles.length} conceptos`
      });
...
function mostrarLogsManual() {
  try {
    SpreadsheetApp.getUi().alert("📊 Ver hojas: " + LOG_SHEET_NAME + " | " + ESTADO_SYNC_SHEET + " | " + SKIPPED_SHEET_NAME);
  } catch (e) {
    Logger.log("Error: " + e);
  }
}
