...
import { ShoppingCart, Plus, TrendingUp, Package, DollarSign, RefreshCw } from 'lucide-react'
...
export const VentasModerno: React.FC = () => {
  ...
  const totalVentasHoy = ventas.filter(v => v.fecha_venta === new Date().toISOString().split('T')[0])
    .reduce((sum, v) => sum + v.subtotal, 0)
  const totalVentasTotal = ventas.reduce((sum, v) => sum + v.subtotal, 0)
  const cantidadProductosVendidos = ventas.reduce((sum, v) => sum + v.cantidad, 0)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8">
      ...
      <div className="transition-all duration-500">
        ...
        {tabActiva === 'historial' && (
          <div className="space-y-6">
            <div className="p-6 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-xl">
              <h2 className="text-xl font-bold text-white mb-4">Historial de Ventas ({ventas.length})</h2>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-6 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-xl">
                ...
                <p className="text-xs text-slate-400">{ventas.filter(v => v.fecha_venta === new Date().toISOString().split('T')[0]).length} venta(s)</p>
              </div>
              ...
              <div className="p-6 bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-xl">
                ...
                <p className="text-xs text-slate-400">{ventas.length} transacción(es)</p>
              </div>
