...
interface UseInsumosStore {
  ...
  agregarInsumo: (insumo: Omit<Insumo, 'id' | 'created_at' | 'updated_at'>) => Promise<void>
 ...
 */
const insertInsumoDirecto = async (data: any) => {
  try {
    const response = await fetch(
      `${supabase.supabaseUrl}/rest/v1/insumos`,
      {
        method: 'POST',
        headers: {
...
export const useInsumos = create<UseInsumosStore>((set, get) => ({
  ...
  agregarInsumo: async (insumo) => {
    ...
    try {
      // ✅ Usar fetch directo
      const { data: insertedData, error: insertError } = await insertInsumoDirecto(insumo)

      if (insertError) throw insertError

      set(state => ({
        insumos: [...state.insumos, insertedData[0] as Insumo]
      }))
    } catch (err) {
