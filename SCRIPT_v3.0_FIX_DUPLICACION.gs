// ==================== GOOGLE APPS SCRIPT v3.0 - FIX DUPLICACIÓN PAGOS ====================
// ✅ FIX: Talonario único por estudiante (sin Date.now()) + validación antes de insertar
...
let SKIPPED_DETAILS = [];
let PAGOS_CREADOS = {}; // ✅ TRACK pagos por DNI para evitar duplicación
...
function sincronizarDesdeGoogleSheets() {
  try {
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🚀 SINCRONIZACIÓN DESDE GOOGLE SHEETS v3.0 (DNI ÚNICO + FIX DUPLICACIÓN)");
...
function sincronizarInteligente() {
  try {
    ...
    Logger.log("\n" + "=".repeat(80));
    Logger.log("🚀 v3.0 - SINCRONIZACIÓN INTELIGENTE (DNI ÚNICO + FIX DUPLICACIÓN)");
    Logger.log("=".repeat(80));
...
function procesarCobranzaPorInstitucionInteligente(config, datosInst) {
  ...
  for (let estudiante of estudiantesConPagos) {
    ...
    if (resultado.success) {
      PAGOS_CREADOS[clavePago] = true; // ✅ MARCAR como procesado
  ...
  for (let r = 1; r < datos.length; r++) {
    ...
    if (montoPagadoInsc > 0) {
      conceptosPagados.push({
        ...
        talonario: String(fila[idxInscTalonario] || "").trim()
      });
    }
    ...
    for (let mes in idxCuota) {
      ...
      if (montoPagado > 0) {
        conceptosPagados.push({
          ...
          talonario: String(fila[idxCuotaTalonario[mes]] || "").trim()
        });
      }
    }
    ...
    for (let mes in idxSeguro) {
      ...
      if (montoPagado > 0) {
        conceptosPagados.push({
          ...
          talonario: String(fila[idxSeguroTalonario[mes]] || "").trim()
        });
      }
    }
...
// ==================== CREAR PAGO MÚLTIPLE (FIX DUPLICACIÓN) ====================
...
function crearPagoMultipleAgrupado(config, estId, estudiante, conceptosPagados, conceptos, config_carreras, carreraId) {
  try {
    ...
    // ✅ FIX: Talonario ÚNICO por estudiante (sin Date.now())
    const talonario = `EST_${estId}_${instId}_2025`;
