function procesarPagosV3(config, pagosArray, tipo, dniAId, conceptos) {
  ...
  for (let pagoArr of pagosArray) {
    try {
      ...
      let talonario = pagoArr[tipo === "INSCRIPCION" ? 11 : (tipo === "SEGURO" ? 12 : 12)] || "N/A";
      metodo = String(metodo).trim().toUpperCase();
      talonario = String(talonario).trim();
      if (!talonario || talonario === "") talonario = "N/A";
  ...
  if (detallesParaInsertar.length > 0) {
    Logger.log(`   ENVIANDO: ${detallesParaInsertar.length} detalles`);
    Logger.log(`   MUESTRA: ${JSON.stringify(detallesParaInsertar[0])}`);
  }
  
  for (let i = 0; i < detallesParaInsertar.length; i += 50) {
    const batch = detallesParaInsertar.slice(i, i + 50);
    try {
      const resp = UrlFetchApp.fetch(`${config.supabaseUrl}/rest/v1/pagos_multiples_detalle`, { 
        method: "post", 
        headers: { "apikey": config.supabaseKey, "Content-Type": "application/json" }, 
        payload: JSON.stringify(batch), 
        muteHttpExceptions: true 
      ...
      if (resp.getResponseCode() === 201 || resp.getResponseCode() === 200) {
        ...
        Logger.log(`     ✅ Batch ${Math.floor(i/50)}: ${batch.length} insertados`);
      } else {
        const errorText = resp.getContentText();
        Logger.log(`     ❌ Batch ${Math.floor(i/50)}: HTTP ${resp.getResponseCode()}`);
        Logger.log(`     ERROR: ${errorText.substring(0, 500)}`);
        res.errores += batch.length;
      }
    } catch (e) {
      Logger.log(`     ❌ Exception batch ${Math.floor(i/50)}: ${e}`);
      res.errores += batch.length;
    }
    Utilities.sleep(DELAY_MS);
  }
