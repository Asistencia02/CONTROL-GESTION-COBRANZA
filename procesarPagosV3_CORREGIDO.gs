function procesarPagosV3(config, pagosArray, tipo, dniAId, conceptos) {
  ...
  for (let key in agrupados) {
    ...
    pagosParaProcesar.push({
      p_institucion_id: pago.estId ? parseInt(config.institucionId) : 0,
...
// ==================== NUEVA FUNCIÓN: Cargar configuracion_carreras ====================
...
function cargarConfiguracionCarreras(config) {
  ...
  try {
    ...
    if (resp.getResponseCode() === 200) {
      ...
      Logger.log(`   📊 Configuración carreras cargada: ${datos.length} total`);
      for (let c of datos) {
...
// ==================== NUEVA FUNCIÓN: Obtener monto original ====================
