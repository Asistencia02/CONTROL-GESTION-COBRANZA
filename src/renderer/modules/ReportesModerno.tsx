...
export const ReportesModerno: React.FC = () => {
    ...
    try {
      ...
      estudiantes.forEach(est => {
        ...
        conceptos?.forEach(concepto => {
          ...
          // ✅ ARREGLADO v2: Solo incluir conceptos que TENGAN mes y año definidos
          ...
          // ❌ NO incluir conceptos sin mes/año (inscripción, seguro sin fecha)
      ...
      const pagosIndividuales = pagos.filter(p => !esAnulado(p)).reduce((sum, p) => sum + p.monto_pagado, 0)
      ...
      try {
        const { data: pagosMultiples } = await supabase
          ...
          `)
          .eq('pagos_multiples.institucion_id', institucionActiva.id)
      ...
      const totalPagosRegistrados = pagosIndividuales + pagosMultiplesTotal

      // ✅ RECAUDABLE = suma de todos los conceptos vencidos - pagos realizados
      // Suma todos los conceptos vencidos (con mes/año y que ya vencieron)
      let totalConceptosVencidos = 0
      if (conceptos && estudiantesActivos > 0) {
        conceptos.forEach(concepto => {
          let debeIncluir = false
          if (concepto.mes && concepto.año) {
            if (concepto.año < anioActual) {
          ...
          if (debeIncluir) {
            totalConceptosVencidos += concepto.monto * estudiantesActivos
          }
        })
      }
      
      const totalRecaudable = totalConceptosVencidos
      
      const porcentajeCobro = totalConceptosVencidos > 0
        ? (totalPagosRegistrados / totalConceptosVencidos) * 100
        : 0
  ...
  // ARREGLADO: Si BD esta vacia, mostrar estado vacio
  if (resumenGlobal.totalRecaudable === 0 && resumenGlobal.estudiantesActivos === 0) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-400 mb-6">No hay estudiantes ni pagos registrados</p>
          <button
            onClick={cargarReportes}
            className="px-6 py-2 bg-cyan-600 hover:bg-cyan-700 text-white rounded-lg font-bold transition-all"
          >
            Recargar
          </button>
        </div>
