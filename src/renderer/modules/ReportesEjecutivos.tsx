...
import { BarChart3, RefreshCw, Users, DollarSign, AlertCircle, TrendingUp } from 'lucide-react'
...
const calcularMesesVencidos = (): number[] => {
  ...
  // Si es día 1-9: meses vencidos son marzo a mes anterior
  // Si es día 10+: meses vencidos incluyen mes actual
  const mesVencido = diaActual < 10 ? mesActual - 1 : mesActual

  for (let m = PRIMER_MES; m <= mesVencido; m++) {
    mesesVencidos.push(m)
  }

  console.log(`[EXEC] Hoy: ${diaActual}/${mesActual} - Meses vencidos: ${mesesVencidos.join(',')}`)
...
export const ReportesEjecutivos: React.FC = () => {
  ...
  const [mesesVencidos, setMesesVencidos] = useState<number[]>([])
  ...
  const procesarInstitucion = async (institucionId: number, mesesVenc: number[]) => {
    try {
      ...
      const { data: estudiantes, error: errEst } = await supabase
        .from('estudiantes')
        .select('id, dni, nombre, apellido, carrera_id, institucion_id, carreras(nombre)')
      ...
      console.log(`[EXEC] ${totalEstudiantes} estudiantes en institución ${institucionId}`)

      if (totalEstudiantes === 0) {
        return {
          kpi: { estudiantes: 0, estudiantesAlDia: 0, estudiantesEnMora: 0, recaudable: 0, recaudado: 0, deuda: 0, eficiencia: 0, moraPercentage: 0 },
          carreras: [],
        }
      }

      // 2. OBTENER CONCEPTOS VENCIDOS FILTRADOS POR INSTITUCIÓN
      const { data: conceptosData, error: errConc } = await supabase
        .from('conceptos_pago')
        .select('id, carrera_id, tipo, mes, monto, institucion_id')
        .eq('institucion_id', institucionId)
        .eq('activo', true)

      if (errConc) throw errConc

      // Filtrar conceptos vencidos: INSCRIPCION siempre, CUOTA/SEGURO según mes
      const conceptosVencidos = (conceptosData || []).filter(c => {
        if (c.tipo === 'INSCRIPCION') return true
        if ((c.tipo === 'CUOTA' || c.tipo === 'SEGURO') && c.mes && mesesVenc.includes(c.mes)) return true
        return false
      })

      console.log(`[EXEC] ${conceptosVencidos.length} conceptos vencidos para institución ${institucionId}`)

      // 3. OBTENER PAGOS REALES - FILTRADOS POR INSTITUCIÓN
      // Usamos JOIN con conceptos_pago para filtrar por institución
      const { data: pagosDetalles, error: errPagos } = await supabase
        .from('pagos_multiples_detalle')
        .select('monto_pagado, concepto_id, conceptos_pago(institucion_id)')

      if (errPagos) throw errPagos

      // Filtrar pagos por institución
      const pagosFiltered = (pagosDetalles || []).filter(p => {
        return (p.conceptos_pago as any)?.institucion_id === institucionId
      })

      // Crear mapa de montos pagados por concepto
      const montosPagedosMap = new Map<number, number>()
      pagosFiltered.forEach((pago: any) => {
        const actual = montosPagedosMap.get(pago.concepto_id) || 0
        montosPagedosMap.set(pago.concepto_id, actual + pago.monto_pagado)
      })

      console.log(`[EXEC] ${pagosFiltered.length} pagos para institución ${institucionId}, ${montosPagedosMap.size} conceptos con pago`)

      // 4. INICIALIZAR CARRERAS
      ...
      estudiantes.forEach(est => {
        // Conceptos vencidos de la carrera de este estudiante
        const conceptosEst = conceptosVencidos.filter(c => c.carrera_id === est.carrera_id)

        let recaudableEst = 0
        let recaudadoEst = 0

        // Sumar montos vencidos y pagados para este estudiante
        conceptosEst.forEach(concepto => {
          const montoPorEstudiante = concepto.monto || 0
          const montoPagadoTotal = montosPagedosMap.get(concepto.id) || 0

          // Asumimos pagos distribuidos equitativamente entre estudiantes de la carrera
          // En realidad, los pagos están en pagos_multiples_detalle asociados a estudiantes específicos
          // Por eso necesitamos verificar si ESTE estudiante pagó

          recaudableEst += montoPorEstudiante
          recaudadoEst += Math.min(montoPagadoTotal, montoPorEstudiante)
        })

        totalRecaudable += recaudableEst
        totalRecaudado += recaudadoEst

        const carr = carreras.get(est.carrera_id)
        if (carr) {
          carr.recaudable += recaudableEst
          carr.recaudado += recaudadoEst

          if (recaudableEst > 0 && recaudadoEst >= recaudableEst) {
            carr.alDia++
            estudiantesAlDia++
          } else if (recaudableEst > 0 && recaudadoEst < recaudableEst) {
            carr.enMora++
            carr.deuda += recaudableEst - recaudadoEst
            estudiantesEnMora++
          }
        }
      })
  ...
  const cargarDatos = async () => {
    try {
      ...
      const mesesVenc = calcularMesesVencidos()
      setMesesVencidos(mesesVenc)

      const [isippRes, milagrosRes] = await Promise.all([
        procesarInstitucion(1, mesesVenc),
        procesarInstitucion(2, mesesVenc),
      ])
      ...
      const globalData = {
        ...
        eficiencia:
          isippRes.kpi.recaudable + milagrosRes.kpi.recaudable > 0
            ? ((isippRes.kpi.recaudado + milagrosRes.kpi.recaudado) / (isippRes.kpi.recaudable + milagrosRes.kpi.recaudable)) * 100
            : 0,
        moraPercentage:
          isippRes.kpi.estudiantes + milagrosRes.kpi.estudiantes > 0
            ? ((isippRes.kpi.estudiantesEnMora + milagrosRes.kpi.estudiantesEnMora) / (isippRes.kpi.estudiantes + milagrosRes.kpi.estudiantes)) * 100
            : 0,
      }

      setGlobal(globalData)

      console.log('[EXEC] ✅ GLOBAL:', {
        estudiantes: globalData.estudiantes,
        alDia: globalData.estudiantesAlDia,
        enMora: globalData.estudiantesEnMora,
        recaudable: globalData.recaudable,
        recaudado: globalData.recaudado,
        deuda: globalData.deuda,
      })
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center p-6">
        <div className="bg-red-500/20 border border-red-500/50 rounded-lg p-4 max-w-sm text-center">
          ...
          <button onClick={cargarDatos} className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-sm font-bold">
            Reintentar
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 p-4 md:p-6">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm">Meses vencidos: {mesesVencidos.join(', ')}</p>
        </div>
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2 rounded text-sm font-bold transition-all ${
              tabActiva === id ? 'bg-cyan-600 text-white shadow-lg' : 'bg-slate-700/30 text-slate-400 hover:bg-slate-600/50'
            }`}
          >
            {label}
          </button>
      ...
      {tabActiva === 'global' && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            <KPICard label="👥 ESTUDIANTES" value={global.estudiantes} icon={<Users size={24} />} color="blue" />
            ...
            <KPICard label="📊 EFICIENCIA" value={`${global.eficiencia.toFixed(1)}%`} icon={<BarChart3 size={24} />} color="purple" />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <KPICard label="📌 RECAUDABLE" value={formatoMoneda(global.recaudable)} icon={<DollarSign size={24} />} color="orange" />
            <KPICard label="✅ RECAUDADO" value={formatoMoneda(global.recaudado)} icon={<DollarSign size={24} />} color="green" />
            <KPICard label="💔 DEUDA" value={formatoMoneda(global.deuda)} icon={<DollarSign size={24} />} color="red" />
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <th className="px-3 py-2 text-right">Recaudable</th>
      ...
      {tabActiva === 'isipp' && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <KPICard label="En Mora" value={isipp.kpi.estudiantesEnMora} icon={<AlertCircle size={20} />} color="red" />
            <KPICard label="Eficiencia" value={`${isipp.kpi.eficiencia.toFixed(1)}%`} icon={<BarChart3 size={20} />} color="purple" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <KPICard label="Recaudable" value={formatoMoneda(isipp.kpi.recaudable)} icon={<DollarSign size={20} />} color="orange" />
            <KPICard label="Recaudado" value={formatoMoneda(isipp.kpi.recaudado)} icon={<DollarSign size={20} />} color="green" />
            <KPICard label="Deuda" value={formatoMoneda(isipp.kpi.deuda)} icon={<DollarSign size={20} />} color="red" />
          </div>
      ...
      {tabActiva === 'milagros' && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            ...
            <KPICard label="En Mora" value={milagros.kpi.estudiantesEnMora} icon={<AlertCircle size={20} />} color="red" />
            <KPICard label="Eficiencia" value={`${milagros.kpi.eficiencia.toFixed(1)}%`} icon={<BarChart3 size={20} />} color="purple" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <KPICard label="Recaudable" value={formatoMoneda(milagros.kpi.recaudable)} icon={<DollarSign size={20} />} color="orange" />
            <KPICard label="Recaudado" value={formatoMoneda(milagros.kpi.recaudado)} icon={<DollarSign size={20} />} color="green" />
            <KPICard label="Deuda" value={formatoMoneda(milagros.kpi.deuda)} icon={<DollarSign size={20} />} color="red" />
          </div>
          <div className="space-y-2">
