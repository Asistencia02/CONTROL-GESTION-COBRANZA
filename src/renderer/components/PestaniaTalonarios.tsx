...
interface PestaniaTalonariosProps {
  institucionId: number
  carreras: any[]
}

export const PestaniaTalonarios: React.FC<PestaniaTalonariosProps> = ({ institucionId, carreras }) => {
  const {
    registrando,
    error: errorGlobal,
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            ...
            <div className="space-y-3">
              ...
              {(filtroNombre || filtroDni) && (
                <button
                  onClick={() => {
                    setFiltroNombre('')
                    setFiltroDni('')
                  }}
                  className="w-full text-xs bg-gray-100 hover:bg-gray-200 text-gray-700 py-2 rounded-lg transition font-semibold"
                >
                  Limpiar filtros
                </button>
              )}
