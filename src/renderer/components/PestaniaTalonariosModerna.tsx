...
export const PestaniaTalonariosModerna: React.FC<PestaniaTalonariosModernaProps> = ({
  ...
  // ✅ FUNCIÓN DE ORDENAMIENTO: INSCRIPCIÓN primero, luego SEGURO+CUOTA por mes
  const ordenarConceptos = (conceptos: ConceptoPago[]) => {
    const inscripcion = conceptos.filter(c => c.tipo?.toUpperCase() === 'INSCRIPCION')
    const otros = conceptos.filter(c => c.tipo?.toUpperCase() !== 'INSCRIPCION')

    // Ordenar otros por mes, pero agrupar SEGURO+CUOTA juntos
    const otrosOrdenados: ConceptoPago[] = []
    const mesesUnicos = [...new Set(otros.map(c => c.mes))].sort((a, b) => (a || 0) - (b || 0))
    
    mesesUnicos.forEach(mes => {
      // Primero SEGURO del mes
      const seguros = otros.filter(c => c.mes === mes && c.tipo?.toUpperCase() === 'SEGURO')
      otrosOrdenados.push(...seguros)
      
      // Luego CUOTA del mes
      const cuotas = otros.filter(c => c.mes === mes && c.tipo?.toUpperCase() === 'CUOTA')
      otrosOrdenados.push(...cuotas)
    })

    return [...inscripcion, ...otrosOrdenados]
  }

  // ✅ ORDENAR ConceptoSeleccionable (para conceptosPendientes) - FILTRADO POR CARRERA
  const ordenarConceptosPendientes = (conceptos: ConceptoSeleccionable[]) => {
    // Obtener carrera del estudiante seleccionado
    const estudianteActual = estudiantes.find(e => e.id === estudianteSeleccionado)
    const carreraEstudiante = estudianteActual?.carrera_id
    
    // Filtrar SOLO conceptos de la carrera del estudiante
    const conceptosCarrera = conceptos.filter(c => c.carrera_id === carreraEstudiante)
    
    const inscripcion = conceptosCarrera.filter(c => c.tipo?.toUpperCase() === 'INSCRIPCION')
    const otros = conceptosCarrera.filter(c => c.tipo?.toUpperCase() !== 'INSCRIPCION')

    const otrosOrdenados: ConceptoSeleccionable[] = []
    const mesesUnicos = [...new Set(otros.map(c => c.mes))].sort((a, b) => (a || 0) - (b || 0))
  ...
  const recargarConceptosDelServidor = async () => {
    try {
      ...
      const nuevosConceptos = ordenarConceptos(conceptosData || [])
      setConceptos(nuevosConceptos)

      if (estudianteSeleccionado) {
        const pendientes = await obtenerConceptosPendientes(institucionId, estudianteSeleccionado, nuevosConceptos)
        setConceptosPendientes(ordenarConceptosPendientes(pendientes))
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        ...
        setConceptos(ordenarConceptos(conceptosData || []))
      } catch (err) {
  ...
  useEffect(() => {
    if (estudianteSeleccionado && conceptos.length > 0) {
      ...
      obtenerConceptosPendientes(institucionId, estudianteSeleccionado, conceptos, carreraDelEstudiante)
        .then((pendientes) => setConceptosPendientes(ordenarConceptosPendientes(pendientes)))
            ...
            <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 p-6 rounded-2xl">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    key={opt.value}
                    onClick={() => setMetodoPago(opt.value as any)}
                    className={`px-4 py-4 rounded-xl border-2 font-semibold transition text-center cursor-pointer ${
                ...
                <div className="mt-4 pt-4 border-t border-slate-700/50">
                  ...
                  <div className="flex gap-3">
                    <button
                      ...
                      className={`flex-1 px-3 py-3 border-2 rounded-lg font-semibold text-sm text-center cursor-pointer transition ${
                        tipoTarjeta === 'CREDITO'
