    ...
    async (
      ...
      try {
        ...
        for (const pago of pagosARegistrar) {
          ...
          // ✅ ARREGLADO: Aplicar descuento de mora a los detalles también
          const detalles = totales.seleccionados
            .map((concepto) => {
              // Calcular la mora del concepto individual
              const moraConcepto = concepto.mora && concepto.mora.estaVencida ? concepto.mora.montoRecargo : 0
              
              // Si se perdona la mora, no incluir el recargo
              const montoPrincipalConcepto = concepto.monto
              ...
              return {
                ...
                monto_original: Number(montoPrincipalConcepto * proporcion),
                monto_mora: Number(moraFinalConcepto * proporcion),
                monto_pagado: Number(montoConceptoProporcional),
                include_mora: concepto.mora ? concepto.mora.estaVencida && !aplicarPerdonMora : false,
