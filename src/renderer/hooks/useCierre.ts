...
export interface CierreDiario {
  ...
  resumen_por_metodo_cantidad: Record<string, number>
  creado_en: string
}
...
interface UseCierreStore {
  ...
  obtenerCierres30Dias: (institucion_id: number) => Promise<CierreDiario[]>
}
...
export const useCierre = create<UseCierreStore>((set) => ({
  ...
  obtenerUltimoCierreDiario: async (institucion_id: number) => {
    ...
    try {
      ...
      const cierre: CierreDiario = {
        ...
        resumen_por_metodo_cantidad: data.resumen_por_metodo_cantidad || {},
  ...
  obtenerCierreDiarios: async (institucion_id: number, fechaInicio: Date, fechaFin: Date) => {
    ...
    try {
      ...
      const cierres: CierreDiario[] = data.map(c => ({
        ...
        resumen_por_metodo_cantidad: c.resumen_por_metodo_cantidad || {},
  ...
  obtenerCierres30Dias: async (institucion_id: number) => {
    set({ loading: true, error: null })
    try {
      const hoy = new Date()
      ...
      if (!data || data.length === 0) {
        console.log(`ℹ️ No hay cierres en los últimos 30 días`)
        return []
      }
      ...
      console.log(`✓ ${cierres.length} cierres obtenidos (últimos 30 días)`)
      return cierres
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Error obteniendo cierres'
      set({ error: message })
      console.error('Error obteniendo cierres 30 días:', err)
      return []
    } finally {
      set({ loading: false })
    }
  ...
  crearCierreDiario: async (institucion_id: number) => {
    ...
    try {
      ...
      const cantidadPorMetodo: Record<string, number> = {}
      ...
      pagosHoy?.forEach(pago => {
        ...
        cantidadPorMetodo[metodo] = (cantidadPorMetodo[metodo] || 0) + 1
      })
      ...
      const cierreDiario: CierreDiario = {
        ...
        resumen_por_metodo_cantidad: cantidadPorMetodo,
