  ...
  cargarDeudasCriticas: async (institucion_id: number) => {
    ...
    try {
      ...
      const { data, error } = await supabase
        .from('v_estudiantes_deuda_critica')
        .select('*')
        .eq('institucion_id', institucion_id)
        .neq('estado', 'NO_VIENE_MAS')
        .neq('estado', 'BECADO_100')

      if (error) throw error

      const { data: conceptos } = await supabase
        .from('conceptos_pago')
        .select('id, mes, año, monto, tipo')
        .eq('institucion_id', institucion_id)
        .eq('activo', true)

      const { data: pagos } = await supabase
        .from('pagos')
        .select('estudiante_id, concepto_id, estado')
        .neq('estado', 'ANULADO')  // Ignorar pagos anulados
      ...
      pagos?.forEach(p => {
        pagosMap.set(`${p.estudiante_id}-${p.concepto_id}`, true)
      })

      pagosMultiples?.forEach((pm: any) => {
        if (pm.pagos_multiples_detalle && Array.isArray(pm.pagos_multiples_detalle)) {
          pm.pagos_multiples_detalle.forEach((detalle: any) => {
            pagosMap.set(`${pm.estudiante_id}-${detalle.concepto_id}`, true)
          })
        }
      })

      const deudasCriticasFiltradas = (data as DeudaCritica[])
        .filter(deuda => {
          let montoAdeuddoTotal = 0
          conceptosFiltrados.forEach(concepto => {
            const tienePago = pagosMap.has(`${deuda.id}-${concepto.id}`)
            const aplicaBeca = esConceptoBeca(concepto.tipo)
            const montoAResponsabilidad = (deuda.estado === 'BECADO_50' && aplicaBeca) ? concepto.monto * 0.5 : concepto.monto
            
            if (!tienePago) {
              montoAdeuddoTotal += montoAResponsabilidad
            }
          })
          return montoAdeuddoTotal > 0
        })

      set({ deudasCriticas: deudasCriticasFiltradas || [] })
    } catch (err) {
